...
capture program drop simreps_bulk
program define simreps_bulk, rclass
	...
	mata: st_store(., st_addvar("double","gapval"), sim_gapfun(st_data(.,"p"), 6, 0.25, 0.75, 0.05))
...
local sizelist_n0 300 600
local sizelist_n1 300 600
local sizelist_n2 400 800
...
forvalues s = 1/`nsizes' {
	...
	di as txt _n "=== BULK power (gap=6 at p=0.25/0.75, discrepant group=2): n0=`n0' n1=`n1' n2=`n2' (total=`ntot') ==="

	simulate p_ks=r(p_ks) p_ad=r(p_ad) p_cvm=r(p_cvm), ///
		reps(`simreps') seed(220`s'): ///
		simreps_bulk `n0' `n1' `n2' `ess_ratio' `testreps'
	...
	quietly gen str12 condition = "bulk"
...
use `results8', clear
save "E:\ArielStuff\wtd_k_simulations\sim3_3grp.dta", replace
di as txt _n "Saved: E:\ArielStuff\wtd_k_simulations\sim3_3grp.dta"
