...
local condlist null bulk
local ncond : word count `condlist'

forvalues c = 1/`ncond' {
	local cond : word `c' of `condlist'

	di as txt _n "=== re=0.2, `cond' condition: n0=600 n1=600 n2=800 (total=2000) ==="

	simulate p_ks=r(p_ks) p_ad=r(p_ad) p_cvm=r(p_cvm), ///
		reps(`simreps') seed(120`c'): ///
...
save "E:\ArielStuff\wtd_k_simulations\sim15_3grp.dta", replace
di as txt _n "Saved: E:\ArielStuff\wtd_k_simulations\sim15_3grp.dta"
